...
//        }
     ...
     *
     * @param boolean $inherit to include data from parent (default off; use only for presentation layer)
     * @return BoundingBox[]
     */
    public function getMergedBoundingBoxes($inherit = false)
    {
        $bboxes = array();
        $latLonBounds = $this->getLatlonBounds($inherit);
        if ($latLonBounds) {
            $bboxes[] = $latLonBounds;
        }
        return array_merge($bboxes, $this->getBoundingBoxes($inherit));
    }
}
